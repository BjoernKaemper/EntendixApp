...
/**
 * Check if an icon exists in the material icon list
 * @param {string} icon - The icon to check
 * @returns {string} - The icon if it exists, otherwise the question mark icon
 */
export default function iconExists(icon: string): string {
  if (materialIconNames.includes(icon)) {
    return icon;
  }
